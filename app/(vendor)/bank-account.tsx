...
export default function VendorBankAccountScreen() {
  ...
  const fetchProfile = useCallback(async () => {
    try {
      const data = await api.get<{ user: { vendorBankAccount?: BankAccount; bankAccount?: BankAccount } }>("/api/vendor/profile");
      const account = data.user?.vendorBankAccount ?? data.user?.bankAccount ?? null;
      if (account) {
        setExisting(account as BankAccount);
      } else {
        setEditing(true);
      }
    } catch {
