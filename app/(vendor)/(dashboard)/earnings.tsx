...
interface Earning {
  ...
  createdAt: string;
  order?: { totalPrice: number; fuelCost: number; createdAt: string };
}
  ...
  const load = useCallback(async (pageNum = 1, append = false) => {
    try {
      const res = await api.get<EarningsResponse>(
        `/api/vendor/earnings?page=${pageNum}&limit=20`
      );
      ...
      <View style={[s.card, { backgroundColor: theme.surface, borderColor: theme.ash }]}>
        <View style={s.cardLeft}>
          ...
          <View>
            <Text style={[s.typeText, { color: theme.text }]}>
              {item.type === "fuel_sale" ? "Fuel Sale" : "Delivery Fee"}
            </Text>
        ...
        <View style={s.cardRight}>
          ...
          <View
            style={[
              s.statusBadge,
              { backgroundColor: isPending ? "#F5C51822" : "#4CAF5022" },
            ]}
          >
            <Text
              style={[
                s.statusText,
                { color: isPending ? "#F5C518" : "#4CAF50" },
              ]}
            >
              {item.status}
            </Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            ...
            <Text style={[s.emptyText, { color: theme.icon }]}>No earnings yet</Text>
            <Text style={[s.emptySubtext, { color: theme.icon }]}>
              Earnings appear here once orders are confirmed and paid.
            </Text>
          </View>
        }
        ListFooterComponent={
