...
export default function VendorPayoutScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [earningsRes, profileRes] = await Promise.all([
        api.get<{ summary: EarningsSummary }>("/api/vendor/earnings?limit=1"),
        api.get<{ user: { vendorBankAccount?: BankAccount; bankAccount?: BankAccount } }>("/api/vendor/profile"),
      ]);
      setSummary(earningsRes.summary ?? { pending: 0, settled: 0 });
      setBankAccount(profileRes.user?.vendorBankAccount ?? profileRes.user?.bankAccount ?? null);
    } catch {
      // silently fail
    } finally {
      setLoading(false);
  ...
  const handleRequestPayout = async () => {
    ...
    if (!bankAccount) {
      toast.error("Add a bank account first", { description: "Go to Profile → Company Info" });
      return;
    }
    setSubmitting(true);
    try {
      const res = await api.post<{ message: string; status: string }>("/api/vendor/withdraw", { amount: parsed });
      toast.success(
  ...
  return (
    <SafeAreaView style={s.safe}>
      ...
      >
        ...
        <View style={s.header}>
          <View>
            <Text style={[s.greeting, { color: theme.icon }]}>Vendor</Text>
          ...
          <View style={s.headerRight}>
            ...
            <ProfileButton onPress={() => router.push("/(vendor)/(dashboard)/profile" as any)} size={40} />
          </View>
        </View>
        ...
        >
          ...
          {loading ? (
            <View style={[s.balanceCard, { backgroundColor: theme.primary }]}>
              <Skeleton width={100} height={13} borderRadius={6} color="rgba(255,255,255,0.25)" />
              <Skeleton width={160} height={36} borderRadius={8} color="rgba(255,255,255,0.3)" style={{ marginTop: 8 }} />
              <Skeleton width={200} height={11} borderRadius={5} color="rgba(255,255,255,0.2)" style={{ marginTop: 4, marginBottom: 16 }} />
              <View style={s.balanceMeta}>
                <View style={s.balanceMetaItem}>
                  <Skeleton width={60} height={15} borderRadius={6} color="rgba(255,255,255,0.3)" />
                  <Skeleton width={40} height={11} borderRadius={5} color="rgba(255,255,255,0.2)" style={{ marginTop: 4 }} />
                </View>
            ...
            <View style={[s.balanceCard, { backgroundColor: theme.primary }]}>
              ...
              <Text style={s.balanceSub}>Settled earnings ready for withdrawal</Text>

              <View style={s.balanceMeta}>
                <View style={s.balanceMetaItem}>
                  <Text style={s.balanceMetaVal}>{fmtCurrency(summary.pending)}</Text>
          ...
          {bankAccount ? (
            <View style={[s.bankCard, { backgroundColor: theme.surface, borderColor: theme.ash }]}>
              <View style={[s.bankIcon, { backgroundColor: theme.tertiary }]}>
                <Ionicons name="card-outline" size={20} color={theme.primary} />
              </View>
              <View style={s.bankInfo}>
                <Text style={[s.bankName, { color: theme.text }]}>{bankAccount.bankName}</Text>
                <Text style={[s.bankDetail, { color: theme.icon }]}>
                  {bankAccount.accountName} · ••••{bankAccount.accountNumber.slice(-4)}
                </Text>
              </View>
              <TouchableOpacity onPress={() => router.push("/(vendor)/bank-account" as any)}>
                <Text style={[s.changeText, { color: theme.primary }]}>Change</Text>
              </TouchableOpacity>
            </View>
          ) : (
            <TouchableOpacity
              style={[s.bankCard, s.bankCardEmpty, { backgroundColor: theme.surface, borderColor: theme.primary + "44", borderStyle: "dashed" }]}
              onPress={() => router.push("/(vendor)/bank-account" as any)}
              activeOpacity={0.8}
            >
              <View style={[s.bankIcon, { backgroundColor: theme.tertiary }]}>
                <MaterialIcons name="add-business" size={20} color={theme.primary} />
              </View>
              <Text style={[s.bankCardEmptyText, { color: theme.primary }]}>Add Bank Account</Text>
        ...
        <View style={s.fieldContainer}>
          ...
          {/* Info note */}
          <View style={[s.note, { backgroundColor: theme.primary + "10", borderColor: theme.primary + "30" }]}>
            <Ionicons name="flash-outline" size={16} color={theme.primary} />
            <Text style={[s.noteText, { color: theme.icon }]}>
              Payouts are processed via Paystack. With a verified bank account, funds arrive within minutes. Only settled earnings can be withdrawn.
            </Text>
          </View>

          {/* Submit */}
          <TouchableOpacity
            style={[s.submitBtn, { backgroundColor: theme.primary, opacity: submitting ? 0.7 : 1 }]}
